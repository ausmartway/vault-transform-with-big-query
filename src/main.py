...
class VaultTransformClient:
    ...
    def __init__(self):
        ...
        # Use client token for production - this should be the token generated by setup-transform.sh
        self.vault_token = os.environ.get('VAULT_CLIENT_TOKEN') or os.environ.get('VAULT_TOKEN')
        self.vault_namespace = os.environ.get('VAULT_NAMESPACE', 'admin')  # Default to admin namespace for HCP
        self.transform_role = os.environ.get('VAULT_TRANSFORM_ROLE', 'creditcard-transform')
        
        if not self.vault_token:
            raise ValueError("VAULT_CLIENT_TOKEN or VAULT_TOKEN environment variable is required")
        
        logger.info(f"Initialized Vault client for {self.vault_url} with namespace: {self.vault_namespace}")
...
def vault_transform_bigquery(request):
    ...
    try:
        ...
        # Check for mode in request data to support gcloud testing
        mode = request_data.get('mode', 'encrypt') if request_data else 'encrypt'
        
        # Route based on path or mode parameter
        if '/encrypt' in path or mode == 'encrypt':
            return encrypt_credit_card(request)
        elif '/decrypt' in path or mode == 'decrypt':
